...
硅基流动 API 代理, 使用边缘函数把硅基流动 API 中转到各种平台。还可以聚合多个硅基流动 API Key，随机选取API Key的使用实现负载均衡。

## Vercel部署(推荐)
[![Deploy to Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/ikun-11451/siliconflow-balance-lite)
...
1. [fork](https://github.com/ikun-11451/siliconflow-balance-lite/fork)本项目
...
[![Deploy to Cloudflare Workers](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/ikun-11451/siliconflow-balance-lite)
...
## Netlify部署
[![Deploy to Netlify](https://www.netlify.com/img/deploy/button.svg)](https://app.netlify.com/start/deploy?repository=https://github.com/ikun-11451/siliconflow-balance-lite)
...
可以使用硅基流动的原生 API 格式进行代理请求。
**Curl 示例:**
```bash
curl -X POST --location 'https://<YOUR_DEPLOYED_DOMAIN>/v1/chat/completions' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer <YOUR_SILICONFLOW_API_KEY_1>,<YOUR_SILICONFLOW_API_KEY_2>' \
--data '{
...
本项目兼容 OpenAI 的 API 格式，你可以通过 `/chat/completions` 端点来发送请求。

**Curl 示例:**
```bash
curl -X POST --location 'https://<YOUR_DEPLOYED_DOMAIN>/chat/completions' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer <YOUR_SILICONFLOW_API_KEY>' \
--data '{
    "model": "Qwen/Qwen2.5-7B-Instruct",
    "messages": [
        {
